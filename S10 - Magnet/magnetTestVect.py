...
#  
  ...
  for i in range(m):
    ...
    rX_glob = np.subtract.outer(gXcoil, gXmagnet-shift)
    rY_glob = np.subtract.outer(gYcoil, gXmagnet)

    r_glob = np.linalg.norm(np.array([rX_glob, rY_glob, np.linspace(-Zmagnet, -Zmagnet, len(gXcoil))]))
